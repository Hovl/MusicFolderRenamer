...
import com.beaglebuddy.mp3.id3v23.frame_body.*;
 ...
 * </table>
 * <p class="beaglebuddy">
 * This class provides methods for reading and writing ID3v2.3 frames.  As the .mp3 file format shows below, an {@link com.beaglebuddy.mp3.id3v23.ID3v23Tag ID3v2.3 tag} is
 ...
 * defines 74 different {@link com.beaglebuddy.mp3.enums.FrameType types of frames}.  A frame consists of a {@link ID3v23FrameHeader header} and a
 * {@link com.beaglebuddy.mp3.id3v23.frame_body.ID3v23FrameBody body}. The frame header contains some information about the frame body (such as its size, type, etc.) and as
 * such does not vary much between the different frame types.  This is in contrast to the {@link com.beaglebuddy.mp3.id3v23.frame_body.ID3v23FrameBody body}, which contains
 ...
 *       <tr><td class="beaglebuddy">2. </td><td class="beaglebuddy">an invalid frame id is encountered                                            </td><td class="beaglebuddy">The <a href="http://www.id3.org">ID3v2.3 Specification</a> defines 74 frame types, each with its own unique 4 character
 *                                                                                                                                                                              {@link com.beaglebuddy.mp3.enums.FrameType frame id}.  If the Beaglebuddy MP3 library encounters a frame with an invalid frame id, then it tries
 ...
 *                                                                                                                                                                              frame and that the remainder of the frames will be valid.  From tests run at Beaglebuddy on about 20,000 mp3 files, this has been the case, and
 ...
 *                                                                                                                                                                              invalid field value(s).  Examples of invalid field values include an invalid {@link com.beaglebuddy.mp3.enums.Language language code}, an invalid
 *                                                                                                                                                                              {@link com.beaglebuddy.mp3.enums.Encoding character encoding} (must be 0 or 1), an invalid {@link com.beaglebuddy.mp3.enums.TimeStampFormat time
...
public class ID3v23Frame
{
    ...
    * @throws IllegalArgumentException   if the class of the frame body does not correspond that sepcified by the <i>frameType</i>.
   ...
   public ID3v23Frame(ID3v23FrameHeader header, InputStream inputStream) throws IOException, IllegalArgumentException
   {
      ...
      catch (NullPointerException ex)
      {
         ...
         if (frameBodyClass == null)
         {
            ...
            invalidMessage = "Invalid ID3 v2.3 frame id " + header.getInvalidFrameId();
         }
         else
         {
