...
public class ID3v23FrameBody extends ID3v23FrameBodyUtility
{
   ...
   public ID3v23FrameBody(InputStream inputStream, String invalidFrameId, int frameBodySize) throws IOException
   {
      ...
      if (inputStream.read(buffer) != buffer.length)
         throw new IOException("Error reading the body of an invalid frame with id " + invalidFrameId + ".");
   }
    ...
    */
   protected int getNextNullTerminator(int startingFrom, Charset charset)
   {
      return getNextNullTerminator(buffer, startingFrom, charset);
   }
   ...
   {
      if (dirty)
         throw new IllegalStateException("The frame " + frameType.getId() + " has been modified and requires setBuffer() to be called before it can be saved.");
